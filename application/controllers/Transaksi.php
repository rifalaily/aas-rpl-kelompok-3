...
class Transaksi extends CI_Controller {
	...
	{    
		...
		// Pass anggota_id and level from session to the view
		$this->data['anggota_id'] = $this->session->userdata('anggota_id');
...
public function prosespinjam()
{
    ...
    if ($this->input->get('pinjam_id')) {
        ...
        $this->session->set_flashdata('pesan', '<div id="notifikasi"><div class="alert alert-warning"><p>  Hapus Transaksi Pinjam Buku Sukses !</p></div></div>');
        redirect(base_url('transaksi/kembali')); // Redirect to the data pengembalian buku page
    }
    ...
    if ($this->input->get('kembali')) {
        ...
        $this->session->set_flashdata('pesan', '<div id="notifikasi"><div class="alert alert-success"><p> Pengembalian Pinjam Buku Sukses !</p></div></div>');
        redirect(base_url('transaksi/kembali')); // Redirect to the data pengembalian buku page
    }
}
...
{
    $this->load->model('M_Admin'); // Load model jika belum diload
    $count = $this->M_Admin->CountTableId('tbl_biaya_denda', 'id_biaya_denda', $id);
    if ($count > 0) {
	...
	{
		$anggota_id = $this->input->post('kode_anggota');
		log_message('debug', 'Fetching data for anggota_id: ' . $anggota_id);
	
		$user = $this->M_Admin->get_tableid_edit('tbl_login', 'anggota_id', $anggota_id);
		error_reporting(0);
		...
		if ($user && $user->nama != null) {
			...
			log_message('debug', 'User not found for anggota_id: ' . $anggota_id);
	...
	{	
		...
		if($row->num_rows() > 0)
		{
			...
			if(!$this->session->has_userdata('cart')) {
				...
				$index = $this->exists($id); // Menggunakan fungsi exists() yang sudah ada
	
				if($index == -1) {
					$cart[] = $item; // Menggunakan sintaks yang lebih sederhana untuk menambahkan item ke array
				} else {
					$cart[$index]['qty']++; // Menggunakan 'qty' bukan 'quantity' sesuai dengan array item yang diinisialisasi
				}
			}
	...
	public function kembalikan_sendiri() {
