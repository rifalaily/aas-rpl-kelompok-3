                  ...
                  <tr>
                      ...
                      <?php 
                        if($isi['status'] == 'Di Kembalikan')
                        {
                          ...
                          if($diff > 0 )
                          {
                            ...
                            echo '<p style="color:green; text-align: left;">Tidak Ada Denda</p>';
                          }
                        }
                      ?>
                    </td>
                    <td style="text-align:center;">
                      <?php if($this->session->userdata('level') == 'Petugas'){ ?>
                        ...
                        <?php }?>
                        <a href="<?= base_url('transaksi/detailpinjam/'.$isi['pinjam_id'].'?pinjam=yes');?>" class="btn btn-primary btn-sm" title="detail pinjam"><i class="fa fa-eye"></i></a>
...
<script>
    $(document).ready(function(){
        $(".kembalikan-sendiri").click(function(){
            var pinjamId = $(this).data("id");
            if(confirm("Apakah Anda yakin ingin mengembalikan buku ini?")){
                $.ajax({
                    url: "<?php echo base_url('transaksi/kembalikan_sendiri'); ?>",
                    type: "POST",
